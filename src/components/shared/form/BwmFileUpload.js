...
// needed to upload - attached to croppedFile
      const {fileURL, blob} = await getCroppedImg(imgRef, crop, 'newfile.jpeg');
// Convert the blob part into a jpeg file
      const croppedFile = new File([blob], "newfile.jpeg", {type: 'image/jpeg'});
  ...
  // Show Upload Image status
    const renderUploadStatus = () => {
      ...
      if(uploadStatus === 'FAIL') {
        return <div className="alert alert-danger">Failed To Upload This Image</div>
      }
    }
////////////////// Render///////////////////////
  ...
  return(
    ...
    <div className='img-upload-container'>
      ...
      <div className="form-group">
        ...
        {
          uploaded &&
            <input type="file" accept=".jpeg, .png, .jpg" className="form-control-file" onChange={onSelectFile} />
        }
      </div>
      {
        imgSrc && uploaded &&
      ...
      {
        croppedImgageUrl && (
          <img alt="Crop" style={{maxWidth: '100%'}} src={croppedImgageUrl}></img>
        )
      }
      {
        file && uploaded &&
          <button className="btn btn-bwm btn-upload" type="button" disabled={!file} onClick={() => uploadThisImage()}>
            Choose this image
          </button>
      }
      {
