...
// reset it to null when leaving
    ...
    const renderRentalCard = () => {
        ...
        <RentalManageCard deleteThisRental={deleteThisRental} key={index} rental={rental}></RentalManageCard>)
    }
    const deleteThisRental = (rentalId) => {
        deleteRental(rentalId)
        .then(res => setRentals([...rentals.filter(rental => rental._id !== rentalId)]))
    ...
    if(rentals && rentals.length === 0){
        return(
            <div className='alert alert-warning'>
                ...
                <Link style={{'marginLeft': '30px'}} className='btn btn-bwm' to='/rentals/create'>Register Rental</Link>
            </div>
        )
    }
