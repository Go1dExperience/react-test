            ...
            <div className={`card text-center ${deleteClass}`}>
                <div className='card-block'>
                    ...
                    {rental.bookings && rental.bookings.length > 0 &&
                        <RentalManageModal bookings={rental.bookings}></RentalManageModal>}               
                </div>
                <div className='card-footer text-muted'>
                    Created at {prettifyDate(rental.createdAt)}
                    {!del && <button onClick={showDelete} className="btn btn-danger">Delete</button>}
                    {
                    del && 
                        <div className="delete-menu">Do You Confirm ? 
                            <button onClick={() => deleteRental(rental._id)} className="btn btn-danger">Yes</button>
                            <button onClick={hideDelete} className="btn btn-primary">No</button>   
                        </div> 
                    }
                </div>
            </div>
